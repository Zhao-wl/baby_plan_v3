...
- [x] 7.1 修改 `TimerState`，添加 `currentRecordId` 字段
- [x] 7.2 修改 `TimerStorageService`，支持 `currentRecordId` 的持久化
- [x] 7.3 修改 `TimerNotifier.start()`，启动计时立即创建数据库记录
- [x] 7.4 修改 `TimerNotifier.stop()`，更新现有记录而不是创建新记录
- [x] 7.5 修改 `TimerNotifier.stopWithForm()`，支持返回记录ID
- [x] 7.6 修改 `TimerNotifier.cancel()`，删除草稿记录
- [x] 7.7 在计时器数据变更处触发 `activityDataChangeProvider` 通知
